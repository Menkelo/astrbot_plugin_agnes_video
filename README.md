# astrbot_plugin_agnes_video

基于 [Agnes AI Video V2.0](https://www.agnes-ai.com/zh-Hans/docs/agnes-video-v20) API 的 AstrBot 视频生成插件，支持文生视频、图生视频和关键帧动画。

## 功能特性

- 文生视频：通过文本提示词直接生成电影级视频。
- 图生视频：消息中附带或引用一张图片，将静态图片转化为动态视频。
- 关键帧动画：引用两张及以上图片，在多个关键帧之间生成流畅过渡。
- 自动分流：单个 `/vgen` 命令根据消息中的图片数量自动选择生成模式。
- 异步任务管理：创建任务后立即返回任务 ID，后台自动轮询，生成完成自动推送视频。
- 适配 QQ 两种图片调用方式：消息中直接附带图片，以及引用/回复一条含图片的消息。

## 安装

1. 将插件目录 `astrbot_plugin_agnes_video` 放入 AstrBot 的 `data/plugins/` 目录。
2. 在 AstrBot WebUI 的「插件管理」中启用插件。
3. 进入插件配置，填写 Agnes AI API Key（申请地址见 Agnes AI 官网）。

依赖：`aiohttp`（requirements.txt 已声明，AstrBot 通常自带）。

## 命令

| 命令 | 触发条件 | 生成模式 |
| --- | --- | --- |
| `/vgen <提示词>` | 无图片 | 文生视频 |
| `/vgen <提示词>` | 消息附带一张图片，或引用含一张图片的消息 | 图生视频 |
| `/vgen <提示词>` | 引用含两张及以上图片的消息 | 关键帧动画 |

说明：

- 图片来源优先级：消息中直接附带的图片 > 引用消息中的图片 > 提示词中粘贴的图片 URL。
- 若只发图片、不写提示词，图生视频 / 关键帧动画会使用内置的默认提示词。
- 提示词中粘贴的多个图片 URL 同样会触发图生视频 / 关键帧动画。
- 图片必须是可公开访问的 URL。QQ（OneBot）平台多数情况下会直接下发图片链接；若协议端未下发链接，插件会尝试通过 AstrBot 文件服务（`callback_api_base`）将图片注册为可访问地址。
- QQ 平台引用图片时，若协议端未下发图片链接，请确认 AstrBot 配置中 `callback_api_base` 为公网可达地址，否则插件将提示你直接粘贴图片 URL。

## 配置项

| 配置 | 默认值 | 说明 |
| --- | --- | --- |
| `api_key` | 无 | Agnes AI API Key（必填） |
| `base_url` | `https://apihub.agnes-ai.com` | API 网关地址 |
| `model` | `agnes-video-v2.0` | 视频生成模型 |
| `width` / `height` | `1152` / `768` | 视频分辨率（16:9 横版） |
| `num_frames` | `121` | 帧数，需 ≤ 441 且满足 8n+1 |
| `frame_rate` | `24` | 帧率 |
| `seed` | `-1` | 随机种子，-1 表示随机 |
| `poll_interval` | `5` | 轮询间隔（秒） |
| `max_poll_time` | `600` | 后台最大等待时间（秒） |

## 参数说明

- 视频时长由 `num_frames / frame_rate` 决定，例如 `121 帧 / 24fps` 约 5 秒。
- 分辨率会自动标准化到 480p / 720p / 1080p 档位，宽高比支持 16:9、9:16、1:1、4:3、3:4。
- 提示词推荐结构：`[主体] + [动作] + [场景] + [镜头运动] + [光线] + [风格]`。

## 使用示例

```
# 文生视频
/vgen 一只猫在沙滩上散步，海浪轻拍，夕阳金色光线，电影感运镜

# 图生视频：发 /vgen 时在消息里附带一张图片
/vgen 人物缓缓转身看向镜头，自然表情

# 图生视频：引用一条含一张图片的消息
/vgen 让图中的人物抬头微笑

# 关键帧动画：引用一条含两张及以上图片的消息
/vgen 在两个关键帧之间平滑过渡，保持角色一致
```

## 注意事项

- 关键帧动画要求消息平台能同时提供多张图片的链接（QQ 官方机器人与 OneBot 均支持；引用图片时依赖 `callback_api_base` 提供公网地址）。
- 生成完成后优先直接推送视频消息；若当前平台不支持视频组件，会自动回退为发送下载链接。
- 部分消息平台对视频大小与时长有限制，长视频可能发送失败，此时可从链接下载。
